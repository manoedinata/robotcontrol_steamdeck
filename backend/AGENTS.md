...
- `WebRTCStream.update_config` returns whether it replaced every connection, and that answer -- not a second guess at the rule in `server.py` -- decides which sources the UI is told to reconnect.
- Applying a config that changes camera transport broadcasts `{ "type": "camera", "streams": [ids] }` naming the sources whose connection was replaced, after the change is in force. The renderer reconnects those feeds; it must not reconnect on saving settings itself, which would race the backend. A config that changes no camera url, credential, backend or transport broadcasts nothing.
...
- Config fields include `udp_host`, `udp_port`, `udp_listen_port`, `camera_streams`, `camera_backend`, and `rtsp_transport`; they never enter UDP payloads. `camera_backend` is `go2rtc` by default or `aiortc`. `rtsp_transport` is `tcp` by default or `udp`, applies to the aiortc dial only, and never to recording. Changing it rebuilds the camera backend only when aiortc is the one running; under go2rtc it is recorded and nothing is torn down, because that backend never reads it. `camera_streams` is a list of `{ "id", "url" }` RTSP sources kept warm at once; ids match `[A-Za-z0-9_-]{1,64}` and are unique. RTSP credentials are URL-encoded in each `url` userinfo and must never be logged.
...
- Releasing an aiortc camera player never happens on the event loop. `stop()` joins the player's worker thread, which is sitting in `demux()` on the camera's socket, and closing the container talks to the camera again: on the loop that stops the 50 Hz sender, telemetry, PTZ and the control socket for seconds, and it happens exactly when a camera has stopped answering. The track is ended on the loop (`_end_tracks`, which is what everything reading the connection sees) and the blocking half runs detached (`_close_player`). Every holder waits on that one teardown.
- Every aiortc connection is drained by its own tap and dropped after `STALL_TIMEOUT_S` without a packet, closing its peers. A camera that goes quiet without disconnecting reaches no error path, so the silence is what has to be timed; under go2rtc the connection is in a child process and the renderer's frame watchdog plus `?restart=1` stand in for this.
- Opening an aiortc player is bounded by `CAMERA_OPEN_TIMEOUT_US`, the same five seconds for an RTSP source and for a relayed WebSocket one, so `/offer` fails fast instead of hanging. The relayed path needs this as much as RTSP does: `detect_format` caches the demuxer per url, so a source that has gone offline since it last connected skips that probe's own timeout and goes straight into the open.
...
- Camera sources are RTSP URLs; an empty or absent `camera_streams` list keeps playback idle. `POST /offer` takes `?src=<stream id>` to pick a stream (optional only when one stream is configured), and `?restart=1` to drop the connection held for that source before answering. Only the renderer can see pictures stop, so only the renderer asks for that.
