...
The application consists of a FastAPI backend that handles camera transport (every configured source kept connected at once for instant switching, whatever its transport), binary UDP command encoding, a 50 Hz command sender, wheel telemetry reception, host ping measurements, simultaneous recording of every camera source, a library for browsing and playing those recordings back, RTSP carried over TCP or UDP as configured, and PTZ camera control (pan/tilt at an operator-set speed, zoom, focus, and the infrared light over ISAPI), plus an Electron + Vue 3 frontend that provides the gamepad/touch/keyboard UI and telemetry HUD.
