...
`distancePerCount` is how far the robot travels per encoder count, in metres. The UDP telemetry packet carries the two wheel positions as raw counts, so the conversion is the renderer's: Home shows their mean times this, in metres up to a kilometre and in kilometres above it. It lives beside the telemetry listen port in Settings, since it describes what arrives on that port. It defaults to `1`, which shows the count as it arrives -- only the operator knows their gearing and wheel size -- and `0` is allowed, reading as "do not show a distance".
