...
On the Home view, both sticks retain robot-control behavior and the D-pad stays with the camera; interface navigation consumes neither. The Settings, Recordings, and Exit buttons are tapped on the touchscreen; focus still returns to the button that opened a page when it closes, but it is the touchscreen that fires it, since A drives the direction toggle here. The distance travelled sits under the telemetry bar, to the right of the pan/tilt speed slider. It is integrated from the two wheel speeds in frames 3 and 4 as differential-drive odometry: each packet's movement is the speed the robot reported held over the time since the last packet, the two wheels are averaged into a step along the robot's heading, and that heading turns with the difference between them. A speed only becomes a distance over an interval, and the only interval available is the gap between arrivals, measured on this side of the network -- so a packet that goes missing is movement the odometry can never account for, unlike the absolute positions in frames 1 and 2. A gap longer than half a second is treated as a break rather than as the robot having held its last speed across it. The step is signed, so reversing winds the distance back rather than adding to it -- what it reads is travel made good, not an odometer. Both measurements it needs, metres per encoder count and the wheel separation, are set in Settings beside the telemetry port.

Holding the distance for 700 ms zeroes it, along with the pose and the counts they are measured from. A hold rather than a tap, because it sits beside a live feed on a touchscreen the operator is already holding, and a stray thumb must not throw away a drive's worth of odometry; the accent fills the bar while the hold runs, so it can be seen coming and let go of, and the bar flashes once when the reset lands. With no speed slider beside it -- a camera the PTZ address does not match -- the distance keeps to the same right-hand column it occupies when there is one, rather than stretching the full width of the telemetry above it.

Under that sits a debug strip: the pose as `x`, `y` and `θ`, then the two encoder frames counted from where the run started. The counts are shown from the run's start rather than absolute so the reset zeroes them too, which is what makes them checkable against the distance -- their mean, times the metres per count, is what it should read. The absolute counts the robot sent are in the strip's tooltip. The strip is temporary and goes when the odometry is trusted. It is a readout, not a control -- there is nothing to reset, because the count belongs to the robot. The battery readout in the top right is tapped too: it shows the robot's battery or the Deck's own, one at a time, and each tap swaps which. This robot's telemetry carries no battery level, so the robot side reads "not reported" until one appears in the packet; the Deck's own is unaffected. It carries no gamepad binding, since the D-pad on Home belongs to the camera.
