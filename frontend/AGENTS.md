...
- `src/composables/useOdometry.js`: differential-drive odometry over the two wheel encoders -- pose (`x`, `y`, `theta`), the signed distance travelled, the counts since the run started, and `reset()`, which Home binds to a hold on the distance readout. It lives here, not in the backend, because the wire carries counts and the conversion needs the operator's own measurements of their robot.
