...
- `electron-components/udp-client.js`: validates the destination, serializes command packets and parses battery replies through the shared codec, and publishes battery updates in the main process.
- `electron-components/packet-schema.js`: shared JSON-driven UDP packet codec used by both the client and the diagnostic peer; compiles `packet-schema.json` into command/reply encoders and decoders with derived byte offsets.
- `packet-schema.json`: repository-root definition of the UDP command and reply packet layout (byte order, header, and per-field name/type/role/default/min/max).
- `udp-server.js`: development-only UDP peer that decodes commands via the shared schema, reports packet timing and every decoded field, and streams the current Linux system battery percentage at 50 Hz to the latest valid sender.
...
The UDP preload contract is `updateUdpVelocity(host, port, velocity)`, `stopUdpVelocity()`, `onUdpBatteryPercentage(callback)`, and `onUdpMetrics(callback)`, where velocity is `[yVelocity, thetaVelocity]`. `ControllerPanel.vue` publishes only the latest reactive state. The main process retains the latest command and sends it every 20 ms. Full renderer navigation, renderer termination, window close, and an explicit stop must clear the scheduler so a stale command cannot continue.

The on-wire command and reply packet layouts are defined in `packet-schema.json` at the repository root and compiled by `electron-components/packet-schema.js` into a shared codec used by `electron-components/udp-client.js`, the main-process scheduler, and `udp-server.js`. The schema declares byte order, per-packet header, and ordered fields with `name`, `type` (`float32`/`float64`/`int8`/`uint8`/`int16`/`uint16`/`int32`/`uint32`), optional `role`, `default`, and `min`/`max`. Byte offsets are derived from field order, so never hand-maintain offsets. The app maps the renderer velocity array onto command fields by the `yVelocity` and `thetaVelocity` roles and reads battery replies by the `batteryPercentage` role; keep those roles present so the UI keeps working. Adding, resizing, or reordering fields is a JSON edit, not a code change; the schema is read at startup, so changes require a restart. The shipped default matches the historical 11-byte command (`ITS` header, Y `float32` at offset `3`, theta `float32` at offset `7`, little-endian) and 7-byte battery reply (`ITS` header, `int32` percentage `0..100` at offset `3`). A missing schema file falls back to this default; malformed JSON or an invalid schema must fail fast at startup. The codec validates the header, field types, numeric ranges, and declared `min`/`max` decode bounds; expose only replies from the configured destination host and port to the renderer. Do not overlap sends. Close the UDP socket during application shutdown.
...
Both relays also report unexpected termination and five seconds of stream inactivity through the narrow `onCameraStreamInterrupted(callback)` preload subscription. `CameraFeed.vue` changes the Home status to a red disconnected/error state and retries the configured source every two seconds after relay interruptions, image load errors, and initial connection failures. It returns to connected only after the renderer loads camera output. Intentional source changes and shutdown must suppress interruption notifications and retries.
